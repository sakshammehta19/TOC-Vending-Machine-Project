...
int main() {
	...
	Student kuby;
	kuby.name = "Kuby";
	kuby.points = 100000; //100,000 Points in Wallet
	...
	while (true) {
		...
		case SELECT:
			...
			cout << "[[[Student]]]" << endl;
			cout << kuby.name << " has " << kuby.points << " points total." << endl;
		...
		case CALCULATE:
			...
			if (payment > kuby.points || payment < cost || payment <= 0) {
				cout << "You don't have enough money!" << endl;
				currentState = SELECT;
				cout << endl << endl;
			}
			else {
				kuby.points = kuby.points - payment;
				int change = payment - cost;
				cout << "Thank you! Your change is " << change << " points." << endl;
				kuby.points = kuby.points + change;
				currentState = UPDATE;
			}
